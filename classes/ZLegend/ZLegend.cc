// @(#)root/graf:$Id: ZLegend.cxx 44072 2012-05-02 15:10:27Z couet $
...
Begin_Macro(source)
{
   ...
   TH1F *h1 = new TH1F("h1","ZLegend Example",200,-10,10);
   ...
   leg = new ZLegend(0.1,0.7,0.48,0.9);
...
Begin_Macro(source)
{
   ...
   ZLegend* leg = new ZLegend(0.2, 0.2, .8, .8);
...
//______________________________________________________________________________
ZLegend::ZLegend(): TPave(), TAttText(12,0,1,gStyle->GetLegendFont(),0)
{
   /* Begin_Html
   Default constructor.
   End_Html */
...
//______________________________________________________________________________
ZLegend::ZLegend( Double_t x1, Double_t y1,Double_t x2, Double_t y2,
        ...
        :TPave(x1,y1,x2,y2,4,option), TAttText(12,0,1,gStyle->GetLegendFont(),0)
{
   ...
   Normal Contructor.
   <p>
   A ZLegend is a Pave with several TLegendEntry(s).
...
//______________________________________________________________________________
ZLegend::ZLegend( const ZLegend &legend ) : TPave(legend), TAttText(legend),
                                            fPrimitives(0)
{
   /* Begin_Html
   Copy constuctor.
  ...
  if (legend.fPrimitives) {
   ...
   ((ZLegend&)legend).Copy(*this);
}


//______________________________________________________________________________
ZLegend& ZLegend::operator=(const ZLegend &lg)
{
   /* Begin_Html
   Assignment operator.
   End_Html */
...
//______________________________________________________________________________
TLegendEntry *ZLegend::AddEntry(const TObject *obj, const char *label, Option_t *option)
{
   /* Begin_Html
...
/////////////////////////////////////////////////
// Custom AddEntry() function color entry text //
/////////////////////////////////////////////////

//______________________________________________________________________________
TLegendEntry *ZLegend::AddEntry(const TObject *obj, Int_t textColor, const char *label, Option_t *option)
{
   /* Begin_Html
   Add a new entry to this legend. "obj" is the object to be represented.
   "label" is the text you wish to associate with obj in the legend.
   ...
   if ( !fPrimitives ) fPrimitives = new TList;
   fPrimitives->Add(newentry);
   newentry->SetTextColor(textColor);
   return newentry;
}


//______________________________________________________________________________
TLegendEntry *ZLegend::AddEntry(const char *name, const char *label, Option_t *option)
{
   /* Begin_Html
...
//______________________________________________________________________________
void ZLegend::Copy( TObject &obj ) const
{
   /* Begin_Html
   ...
   TPave::Copy(obj);
   TAttText::Copy((ZLegend&)obj);
   ((ZLegend&)obj).fEntrySeparation = fEntrySeparation;
   ((ZLegend&)obj).fMargin = fMargin;
   ((ZLegend&)obj).fNColumns = fNColumns;
}


//______________________________________________________________________________
void ZLegend::DeleteEntry()
{
   /* Begin_Html
...
//______________________________________________________________________________
void ZLegend::Draw( Option_t *option )
{
   /* Begin_Html
...
//______________________________________________________________________________
void ZLegend::EditEntryAttMarker()
{
   /* Begin_Html
...
//______________________________________________________________________________
TLegendEntry *ZLegend::GetEntry() const
{
   /* Begin_Html
...
//______________________________________________________________________________
const char *ZLegend::GetHeader() const
{
   /* Begin_Html
...
//______________________________________________________________________________
void ZLegend::InsertEntry( const char* objectName, const char* label, Option_t* option)
{
   /* Begin_Html
...
//______________________________________________________________________________
void ZLegend::Paint( Option_t* option )
{
   /* Begin_Html
...
//______________________________________________________________________________
void ZLegend::SetNColumns(Int_t nColumns)
{
   /* Begin_Html
   ...
   if(nColumns < 1) {
      Warning("ZLegend::SetNColumns", "illegal value nColumns = %d; keeping fNColumns = %d", nColumns, fNColumns);
      return;
   }
   fNColumns = nColumns;
}
...
void ZLegend::PaintPrimitives()
{
   ...
   // iterate over and paint all the ZLegendEntries
   TIter next(fPrimitives);
   TLegendEntry *entry;
   Int_t iColumn = 0;
   while (( entry = (TLegendEntry *)next() )) {
      ...
      // if the user hasn't set a parameter, then set it to the ZLegend value
...
//______________________________________________________________________________
void ZLegend::Print( Option_t* option ) const
{
   /* Begin_Html
   Dump this ZLegend and its contents.
   End_Html */
...
//______________________________________________________________________________
void ZLegend::RecursiveRemove(TObject *obj)
{
   /* Begin_Html
...
//______________________________________________________________________________
void ZLegend::SavePrimitive(ostream &out, Option_t* )
{
   /* Begin_Html
   ...
   out << "   " << endl;
   char quote = '"';
   if ( gROOT->ClassSaved( ZLegend::Class() ) ) {
      out << "   ";
   } else {
      out << "   ZLegend *";
   }
   // note, we can always use NULL header, since its included in primitives
   out << "leg = new ZLegend("<<GetX1NDC()<<","<<GetY1NDC()<<","
       <<GetX2NDC()<<","<<GetY2NDC()<<","
...
//______________________________________________________________________________
void ZLegend::SetEntryLabel( const char* label )
{
   /* Begin_Html
...
//______________________________________________________________________________
void ZLegend::SetEntryOption( Option_t* option )
{
   /* Begin_Html
...
//______________________________________________________________________________
void ZLegend::SetHeader( const char *header )
{
   /* Begin_Html
   ...
   first->SetTextFont(GetTextFont()); // default font is ZLegend font for the header
   first->SetTextSize(0);
